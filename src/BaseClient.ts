...
export type BaseClientOptions = {
    environment?: core.Supplier<environments.MailchimpEnvironment | string>;
...
export function normalizeClientOptions<T extends BaseClientOptions = BaseClientOptions>(
    ...
    const headers = mergeHeaders(
        {
            ...
            "User-Agent": "@mailchimp/mailchimp-marketing/0.0.146",
    ...
    if (options?.serverPrefix != null) {
        const _serverPrefix = options?.serverPrefix ?? "us1";
        if (baseUrl == null) {
            const _environmentUrls = new Map<unknown, string>([
                [environments.MailchimpEnvironment.Default, `https://${_serverPrefix}.api.mailchimp.com`],
            ]);
            baseUrl = _environmentUrls.get(options?.environment) ?? `https://${_serverPrefix}.api.mailchimp.com`;
        }
    }
